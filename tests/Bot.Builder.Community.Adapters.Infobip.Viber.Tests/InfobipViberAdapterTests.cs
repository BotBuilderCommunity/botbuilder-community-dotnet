...
using Bot.Builder.Community.Adapters.Infobip.Viber.Tests.Framework;
...
{
    public class InfobipViberAdapterTests
    {
        [Fact]
        ...
        public async Task Process_SingleMessage()
        {
            ...
            var bot = new TestBot
            {
                OnMessageActivity = async (x, t, c) =>
                {
                    ...
                    var activity = CreateMessageActivity($"message-{x}");
        ...
        public async Task Process_MultipleMessagesTogether()
        {
            ...
            var bot = new TestBot
            {
                ...
                {
                    // Bot responds with multiple messages separately.
                    var responses = await t.SendActivitiesAsync(new[] { CreateMessageActivity("message1"), CreateMessageActivity("message2"), CreateMessageActivity("message3") }, c).ConfigureAwait(false);
        ...
        public async Task Process_OneActivityWithMultipleInfobipMessages()
        {
            ...
            activity.AddInfobipViberMessage(new InfobipOmniViberMessage{ImageUrl = "image url"});
            activity.AddInfobipViberMessage(new InfobipOmniViberMessage{Text = "text"});
        ...
        public async Task Process_TwoActivitiesWithMultipleInfobipMessages()
        {
            ...
            firstActivity.AddInfobipViberMessage(new InfobipOmniViberMessage{Text = "Text one"});
            activities.Add(firstActivity);
            var secondActivity = CreateMessageActivity();
            secondActivity.AddInfobipViberMessage(new InfobipOmniViberMessage{Text = "Text two"});
        ...
        private Activity CreateMessageActivity(string text = null)
        {
            var message = new Activity();
            message.Type = ActivityTypes.Message;
            message.Id = Guid.NewGuid().ToString();
            message.Recipient = new ChannelAccount { Id = "viber" };
            message.ChannelId = InfobipViberConstants.ChannelName;
            message.Text = string.IsNullOrEmpty(text) ? null : text;
        ...
        private InfobipIncomingMessage<InfobipViberIncomingResult> GetInfobipIncomingMessage()
        {
            return new InfobipIncomingMessage<InfobipViberIncomingResult>
            {
