...
namespace Bot.Builder.Community.Components.Handoff.Channel.DataStore.InMemory
{
    /// <summary>
    ...
    public class InMemoryRoutingDataStore : IRoutingDataStore
    {
            ...
            if (conversationReferenceToRemove.User != null)
            {
                return (Users as List<ConversationReference>)
                    .RemoveAll(conversationReference =>
                        conversationReference.Match(conversationReferenceToRemove)) > 0;
            }
        ...
        public IList<ConversationReference> GetHandoffChannels()
        {
        ...
        public bool MakeConversationRefAvailableForHandoff(ConversationReference conversationRefToAdd)
        {
            AggregationChannels.Add(conversationRefToAdd);
            return true;
        }

        public bool MakeConversationRefUnavailableForHandoff(ConversationReference conversationRefToRemove)
        {
            return ((AggregationChannels as List<ConversationReference>)
                .RemoveAll(conversationReference =>
                    conversationReference.Match(conversationRefToRemove)) > 0);
        }
