...
{
    /// <summary>
    /// A <see cref="BotAdapter"/> that can connect to Twilio whats app service.
    /// </summary>
    public class TwilioWhatsAppAdapter : BotAdapter, IBotFrameworkHttpAdapter
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="TwilioWhatsAppAdapter"/> class using configuration settings.
        /// </summary>
        /// <remarks>
        ...
        /// </remarks>
        /// <param name="twilioClientWrapperOptions"></param>
        /// <param name="logger">The ILogger implementation this adapter should use.</param>
        public TwilioWhatsAppAdapter(TwilioClientWrapperOptions twilioClientWrapperOptions,ILogger logger = null)
        {
            if (twilioClientWrapperOptions == null)
                throw new ArgumentException(nameof(twilioClientWrapperOptions));

            _twilioClient = new TwilioClientWrapper(twilioClientWrapperOptions);

            _options = new TwilioAdapterOptions
            {
                ValidateIncomingRequests = twilioClientWrapperOptions.ValidateIncomingRequests
            };
        ...
        public async Task ProcessAsync(HttpRequest httpRequest, HttpResponse httpResponse, IBot bot, CancellationToken cancellationToken)
        {
            ...
            {
                var query = await bodyStream.ReadToEndAsync().ConfigureAwait(false);

                bodyDictionary = !string.IsNullOrEmpty(query) ? TwilioHelper.QueryStringToDictionary(query) : TwilioHelper.QueryStringToDictionary(httpRequest.Form);
            }


            if (_options.ValidateIncomingRequests && !_twilioClient.ValidateSignature(httpRequest, bodyDictionary))
            {
                throw new AuthenticationException("WARNING: Web hook received message with invalid signature. Potential malicious behavior!");
            }
        ...
        /// configure a <see cref="TwilioWhatsAppAdapter"/>.
        /// </summary>
        /// <param name="configuration"><see cref="IConfiguration"/> to verify for settings.</param>
        /// <returns>A value indicating whether the configuration has the necessary settings required to create a <see cref="TwilioWhatsAppAdapter"/>.</returns>
