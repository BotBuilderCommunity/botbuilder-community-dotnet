...
{
    public class GuidPrompt : Prompt<string>
    {
        public GuidPrompt(string dialogId, PromptValidator<string> validator = null, string defaultLocale = null) 
        ...
        protected override Task<PromptRecognizerResult<string>> OnRecognizeAsync(ITurnContext turnContext, IDictionary<string, object> state, PromptOptions options, CancellationToken cancellationToken = new CancellationToken())
        {
            ...
            if (turnContext.Activity.Type == ActivityTypes.Message)
            {
                ...
                var recognizeGuid = Microsoft.Recognizers.Text.Sequence.SequenceRecognizer.RecognizeGUID(message.Text, culture);
                if (recognizeGuid?.Count > 0)
                {
                    result.Succeeded = true;
                    result.Value = recognizeGuid[0].Resolution["value"].ToString();
                }
            }
